...
def summarize_batch(texts: List[str], model_pipe, batch_size: int = 8, **gen_kwargs) -> List[str]:
    ...
    for i in tqdm(range(0, len(texts), batch_size), desc = "Summarizing batches"):      ## Stepped range for batching
        ...
        outs = model_pipe(batch, truncation = True, **gen_kwargs)       ## outs is a list of dicts when input list -> flatten accordingly
        for o in outs:
            summaries.append(o["summary_text"].strip())
    return summaries

def summarize_truncated_files(truncated_json_path: str, out_name: Optional[str] = None, model_pipe = None, batch_size: int = 8, gen_kwargs: Optional[Dict[str, Any]] = None) -> List[dict]:
    ...
    if gen_kwargs is None:
        gen_kwargs = {"max_length": 256, "min_length": 50}
...
def summarize_full_pairs(pairs_file: str, out_name: Optional[str] = None, model_pipe = None, batch_size: int = 8, gen_kwargs: Optional[Dict[str, Any]] = None) -> List[dict]:
